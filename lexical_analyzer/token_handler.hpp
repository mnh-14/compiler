...
void handle_no_attribute_token(std::string token, std::string lexeme, int line_no){
    declared::token_file << token << ' ';
    declared::log_file << "Line no " << line_no << ": Token " << token << " Lexeme " << lexeme << " found" << std::endl;
}

std::string handle_attributed_token(std::string token_type, std::string lexeme, int line_no, bool insert_symbol=false){
    std::string token = tokens::construct(token_type, lexeme);
    declared::token_file << token << " ";
    declared::log_file << "Line no " << line_no << ": " << tokens::log_string(token_type, lexeme) << " found" << std::endl;
